...
<div class="admin-panel">
    ...
    <div class="panel-body">
        <form id="couponEditForm" action="{{ route('vendor.coupons.update', $coupon->id) }}" method="POST" onsubmit="submitCouponEditForm(event)">
            @csrf
            @method('PUT')
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="code" class="form-control" placeholder="e.g. SAVE50" value="{{ old('code', $coupon->code) }}" required style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: #fff; padding: 10px 14px; border-radius: 8px; font-weight: 700; text-transform: uppercase; outline: none;">
                </div>
                <div class="col-md-6">
                    ...
                    <input type="number" step="0.01" min="0" name="discount" class="form-control" placeholder="e.g. 50.00" value="{{ old('discount', $coupon->discount) }}" required style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: #fff; padding: 10px 14px; border-radius: 8px; outline: none;">
                </div>
                
                <div class="col-12">
                    <label style="display: block; color: #f8fafc; font-size: 0.85rem; font-weight: 600; margin-bottom: 8px;">Coupon Description</label>
                    <textarea name="description" class="form-control" rows="3" placeholder="Describe the coupon conditions, terms, and rules..." style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: #fff; padding: 10px 14px; border-radius: 8px; outline: none;">{{ old('description', $coupon->description) }}</textarea>
                </div>

                <div class="col-md-6">
                    <label style="display: block; color: #f8fafc; font-size: 0.85rem; font-weight: 600; margin-bottom: 8px;">From Date</label>
                    <input type="date" name="valid_from" min="{{ date('Y-m-d') }}" class="form-control" value="{{ old('valid_from', $coupon->valid_from) }}" style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: #fff; padding: 10px 14px; border-radius: 8px; color-scheme: dark; outline: none;">
                </div>
                <div class="col-md-6">
                    <label style="display: block; color: #f8fafc; font-size: 0.85rem; font-weight: 600; margin-bottom: 8px;">To Date</label>
                    <input type="date" name="valid_to" min="{{ date('Y-m-d') }}" class="form-control" value="{{ old('valid_to', $coupon->valid_to) }}" style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: #fff; padding: 10px 14px; border-radius: 8px; color-scheme: dark; outline: none;">
                </div>

                <div class="col-md-6">
                    <label style="display: block; color: #f8fafc; font-size: 0.85rem; font-weight: 600; margin-bottom: 8px;">Minimum Booking Amount ($)</label>
                    <input type="number" step="0.01" min="0" name="min_booking_amount" class="form-control" placeholder="e.g. 200.00" value="{{ old('min_booking_amount', $coupon->min_booking_amount) }}" style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: #fff; padding: 10px 14px; border-radius: 8px; outline: none;">
                </div>
                <div class="col-md-6">
                    <label style="display: block; color: #f8fafc; font-size: 0.85rem; font-weight: 600; margin-bottom: 8px;">Availability Count</label>
                    <input type="number" min="1" name="availability_count" class="form-control" placeholder="e.g. 25" value="{{ old('availability_count', $coupon->availability_count) }}" style="background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: #fff; padding: 10px 14px; border-radius: 8px; outline: none;">
                </div>
...
function submitCouponEditForm(e) {
    ...
    $.ajax({
        url: $('#couponEditForm').attr('action'),
        type: 'POST',
        ...
        error: function(xhr) {
            btn.prop('disabled', false).css('opacity', '1');
            var msg = 'Failed to update coupon.';
