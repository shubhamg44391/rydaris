...
@section('title', $seo_title ?? 'Extras Management')
...
@section('main-content')
    ...
    <div class="admin-panel">
        ...
        <div id="extrasTableContainer">
            @section('extras_table_section')
                    ...
                    <tbody>
                        ...
                        @forelse ($extras as $extra)
                            <tr>
                                ...
                                <td>
                                    @if($extra->status)
                                        <span class="badge" style="background: #dcfce7; color: #067647; padding: 4px 8px; border-radius: 12px; font-weight: bold; font-size: 0.8rem; cursor: pointer;" onclick="toggleExtraStatus({{ $extra->id }}, this)">Active</span>
                                    @else
                                        <span class="badge" style="background: #f1f5f9; color: #64748b; padding: 4px 8px; border-radius: 12px; font-weight: bold; font-size: 0.8rem; cursor: pointer;" onclick="toggleExtraStatus({{ $extra->id }}, this)">Inactive</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div class="table-actions" style="display: flex; gap: 8px;">
                                        ...
                                        <a href="{{ route('vendor.extras.edit', $extra->id) }}" class="icon-button" title="Edit" style="display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border: 1px solid #d7e0e8; border-radius: var(--radius); color: #0f766e; background: #ffffff; text-decoration: none;">
                                        ...
                                        <button type="button" class="icon-button delete-btn" title="Delete" onclick="deleteExtra({{ $extra->id }})" style="display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border: 1px solid #fee2e2; border-radius: var(--radius); color: #ef4444; background: #ffffff; cursor: pointer; padding: 0;">
...
<script>
    function fetchExtras() {
        $('#extrasTableContainer').css('opacity', '0.5');
        $.ajax({
            url: '{{ route("vendor.extras.index") }}',
            type: 'GET',
            dataType: 'json',
            headers: { 'X-Requested-With': 'XMLHttpRequest' },
            success: function(response) {
                if (response.status === 'success' && response.html) {
                    $('#extrasTableContainer').html(response.html).css('opacity', '1');
                }
            },
            error: function() {
                $('#extrasTableContainer').css('opacity', '1');
            }
        });
    }

    function toggleExtraStatus(id, el) {
        $.ajax({
    ...
    function deleteExtra(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "This will delete the extra item. You won't be able to revert this!",
            icon: 'warning',
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Deleted!',
                                text: response.message || 'Extra deleted successfully.',
                                timer: 2000,
                                showConfirmButton: false
                            });
                            fetchExtras();
                        } else {
                            Swal.fire('Error', response.message || 'Failed to delete extra.', 'error');
                        }
                    }
                });
            }
            ...
            html: `
                <div style="text-align: left; padding-top: 10px;">
                    <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
                        ...
                        <div>
                            <div style="color:#94a3b8;font-size:0.83rem;">Vehicle Extra Add-on</div>
                        </div>
                    </div>
