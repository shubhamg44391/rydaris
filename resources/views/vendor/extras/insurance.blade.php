...
@section('main-content')
    <div class="admin-panel">
        <div class="panel-head d-flex justify-content-between align-items-center" style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <div>
                <a href="{{ route('vendor.insurance.create') }}" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 4px;">
                ...
                <tbody>
                    ...
                    @forelse ($insurances as $ins)
                        <tr>
                            ...
                            <td>
                                @if($ins->status)
                                    <span class="badge" style="background: #dcfce7; color: #067647; padding: 4px 8px; border-radius: 12px; font-weight: bold; font-size: 0.8rem; cursor: pointer;" onclick="xpToggle({{ $ins->id }}, this)">Active</span>
                                @else
                                    <span class="badge" style="background: #f1f5f9; color: #64748b; padding: 4px 8px; border-radius: 12px; font-weight: bold; font-size: 0.8rem; cursor: pointer;" onclick="xpToggle({{ $ins->id }}, this)">Inactive</span>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="table-actions" style="display: flex; gap: 8px;">
                                    ...
                                    <a href="{{ route('vendor.insurance.edit', $ins->id) }}" class="icon-button" title="Edit" style="display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; border: 1px solid #d7e0e8; border-radius: var(--radius); color: #0f766e; background: #ffffff;">
                                    ...
                                    <form action="{{ route('vendor.extras.destroy', $ins->id) }}" method="POST" class="delete-form" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
            ...
            html: `
                <div style="text-align: left; padding-top: 10px;">
                    <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;">
                        ...
                        <div>
                            <div style="color:#64748b;font-size:0.83rem;">Insurance Package</div>
                        </div>
                    </div>
