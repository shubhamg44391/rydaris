...
@section('main-content')
    <div class="admin-panel">
        <div class="panel-head" style="margin-bottom: 25px;">
            <div>
                <h2 class="panel-title" style="margin: 0;">Add New Customer</h2>
                <p style="color: #94a3b8; font-size: 0.85rem; margin-top: 4px; margin-bottom: 0;">Create a new customer account directly under your vendor profile.</p>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('vendor.customers.store') }}">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    ...
                    <div>
                        ...
                        <input type="text" class="form-control form-input-custom @error('name') is-invalid @enderror" id="name" name="name"
                            value="{{ old('name') }}" required placeholder="Enter last name" />
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="email" class="form-control form-input-custom @error('email') is-invalid @enderror" id="email" name="email"
                        value="{{ old('email') }}" required placeholder="customer@example.com" />
                    @error('email')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="tel" id="reg_phone" class="form-control form-input-custom" placeholder="Phone number" value="{{ old('country_code') }}{{ old('contact_number') }}" required style="width: 100%;">
