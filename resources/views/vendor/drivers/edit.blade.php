...
@section('main-content')
    <div class="admin-panel">
        <div class="panel-head d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="panel-title">Edit Driver: {{ $driver->name }}</h2>
            </div>
        </div>

        <div class="panel-body">
            <form id="driverEditForm" method="POST" action="{{ route('vendor.drivers.update', $driver->id) }}" enctype="multipart/form-data" novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" class="form-control form-input-custom @error('name') is-invalid @enderror" id="name" name="name"
                            value="{{ old('name', $driver->name) }}" required placeholder="Enter driver full name" />
                        @error('name')
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="email" class="form-control form-input-custom @error('email') is-invalid @enderror" id="email" name="email"
                            value="{{ old('email', $driver->email) }}" placeholder="driver@example.com" />
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" class="form-control form-input-custom @error('license_number') is-invalid @enderror" id="license_number" name="license_number"
                            value="{{ old('license_number', $driver->license_number) }}" placeholder="e.g. DL-1420110012345" />
                    ...
                    <div class="col-md-12 mb-4">
                        ...
                        <textarea class="form-control form-input-custom @error('address') is-invalid @enderror" id="address" name="address" rows="3" required placeholder="Enter driver full residential address...">{{ old('address', $driver->address) }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12 mb-4">
                        ...
                        <textarea class="form-control form-input-custom @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2" placeholder="Any additional notes about driver experience, background check...">{{ old('notes', $driver->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12 mb-4">
                        ...
                        <div id="upload-box" style="border: 2px dashed var(--line, #cbd5e1); border-radius: var(--radius, 10px); padding: 24px; text-align: center; background: var(--bg-2, #f8fafc); cursor: pointer; transition: border-color 0.2s;" onclick="document.getElementById('photo').click();">
                            ...
                            <div id="upload-prompt" style="{{ $driver->photo ? 'display: none;' : 'display: block;' }}">
                                <svg viewBox="0 0 24 24" style="width: 40px; height: 40px; fill: none; stroke: var(--brand, #52ead2); stroke-width: 2; margin-bottom: 8px;">
                                    <circle cx="12" cy="7" r="4"></circle>
                                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                                </svg>
                                <p style="margin: 0 0 4px 0; font-weight: 700; color: var(--text, #1e293b); font-size: 0.95rem;">Click to upload or change driver photo</p>
                                <span style="color: var(--muted, #64748b); font-size: 0.82rem;">Supported formats: PNG, JPG, JPEG, WEBP (Max 5MB)</span>
                            </div>

                            <!-- Image Preview Box -->
                            <div id="preview-box" style="{{ $driver->photo ? 'display: flex;' : 'display: none;' }} flex-direction: column; align-items: center; justify-content: center; text-align: center;">
                                <img id="preview-img" src="{{ $driver->photo ? asset('storage/' . $driver->photo) : '#' }}" alt="Driver Photo Preview" style="max-height: 160px; width: 160px; height: 160px; border-radius: 50%; border: 2px solid var(--brand, #52ead2); object-fit: cover; margin: 0 auto 12px auto; display: block;" />
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#driverEditForm').on('submit', function (e) {
            ...
            $submitBtn.prop('disabled', true).html('<i class="fa-solid fa-spinner fa-spin me-1"></i> Updating Driver...');
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        Swal.fire({
                            title: 'Success!',
                            text: data.message || 'Driver updated successfully.',
                            icon: 'success',
                        ...
                        Swal.fire('Error!', data.message || 'Failed to update driver.', 'error');
                    }
                },
                error: function (xhr) {
                    $submitBtn.prop('disabled', false).html(originalText);
                    ...
                    if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                        ...
                        var msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'An error occurred while updating driver.';
