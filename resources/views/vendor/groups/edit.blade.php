...
@section('main-content')
    <div class="admin-panel">
        <div class="panel-head">
            <div>
                <h2 style="font-size: 1.4rem; color: #111827; margin: 0;">Edit Vehicle Group Details</h2>
                <p class="panel-muted" style="margin: 4px 0 0; font-size: 0.9rem;">Update the name and description of the vehicle group.</p>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('vendor.groups.update', $group->id) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                        value="{{ old('name', $group->name) }}" required placeholder="Enter group name" style="border: 1px solid #d7e0e8; border-radius: var(--radius); padding: 12px; font-size: 0.95rem;" />
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                        rows="5" placeholder="Enter group description" style="border: 1px solid #d7e0e8; border-radius: var(--radius); padding: 12px; font-size: 0.95rem;">{{ old('description', $group->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
