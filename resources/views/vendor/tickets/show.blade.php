    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div style="display: flex; gap: 10px;">
            @if($ticket->status === 'open')
                <form action="{{ route('vendor.support-tickets.close', $ticket->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="btn btn-danger" style="background: #ef4444; border-color: #ef4444; font-weight: 600; padding: 10px 20px; border-radius: 6px;">
                        Close Ticket
                    </button>
                </form>
            @endif
                ...
                <div style="padding: 24px; max-height: 500px; overflow-y: auto; display: flex; flex-direction: column; gap: 20px;">
                    ...
                    @foreach($ticket->replies as $reply)
                        ...
                        <div style="display: flex; flex-direction: column; align-items: {{ $isVendor ? 'flex-end' : 'flex-start' }}; width: 100%;">
                            <div style="background: {{ $isVendor ? 'rgba(59, 130, 246, 0.1)' : 'rgba(255,255,255,0.03)' }}; border: 1px solid {{ $isVendor ? 'rgba(59, 130, 246, 0.2)' : 'rgba(255,255,255,0.05)' }}; border-radius: {{ $isVendor ? '12px 12px 0 12px' : '12px 12px 12px 0' }}; padding: 16px; max-width: 85%;">
                                <span style="font-weight: 700; color: {{ $isVendor ? '#3b82f6' : '#94a3b8' }}; font-size: 0.8rem; display: block; margin-bottom: 6px;">
