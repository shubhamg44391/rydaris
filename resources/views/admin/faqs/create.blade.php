...
@section('main-content')
    <div class="admin-panel">
        <div class="panel-head">
            <div>
                <h2 class="panel-title">Create FAQ</h2>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('admin.faqs.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control form-input-custom @error('title') is-invalid @enderror" id="title" name="title"
                        value="{{ old('title') }}" required placeholder="e.g., Can Rydaris handle multiple rental branches?" style="border: 1px solid var(--line); border-radius: var(--radius); padding: 12px; font-size: 0.95rem; width: 100%; background: var(--bg-2); color: var(--text);" />
                    @error('title')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control form-input-custom @error('description') is-invalid @enderror" id="description" name="description"
                        rows="6" required placeholder="Enter the FAQ answer here..." style="border: 1px solid var(--line); border-radius: var(--radius); padding: 12px; font-size: 0.95rem; width: 100%; background: var(--bg-2); color: var(--text);"></textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
