<nav class="admin-nav">
    ...
    <div class="admin-nav-group">
        <a href="javascript:void(0);" class="nav-toggle {{ Request::is('admin/settings*') || Request::is('admin/profile*') ? 'active' : '' }}" onclick="toggleSubmenu(this)" style="justify-content: space-between; display: flex; align-items: center; gap: 10px; min-height: 42px; padding: 10px 12px; border-radius: var(--radius); font-size: 0.92rem; font-weight: 780; transition: background 0.2s; text-decoration: none;">
            ...
            <svg class="chevron" viewBox="0 0 24 24" style="width: 14px; height: 14px; transition: transform 0.2s ease; {{ Request::is('admin/settings*') || Request::is('admin/profile*') ? 'transform: rotate(180deg);' : '' }}"><path d="m6 9 6 6 6-6"/></svg>
        </a>
        <div class="admin-submenu" style="{{ Request::is('admin/settings*') || Request::is('admin/profile*') ? 'display: flex;' : 'display: none;' }} padding-left: 20px; margin-top: 4px; flex-direction: column; gap: 4px;">
            <a href="{{ route('admin.profile.index') }}" class="submenu-item {{ Request::is('admin/profile*') ? 'active' : '' }}" style="text-decoration: none;">
                <span class="dot"></span>
                Admin Profile
            </a>
