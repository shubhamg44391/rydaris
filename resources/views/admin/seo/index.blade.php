...
@section('main-content')
    ...
    <div style="margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
        <div>
            <h4 style="margin: 0; color: var(--heading-color, var(--text-color, #fff)); font-weight: 700; font-size: 1.4rem;">SEO Page Metadata</h4>
    ...
    <div class="admin-panel" style="border: 1px solid var(--line); border-radius: var(--radius); background: var(--panel); padding: 20px;">
        <div class="panel-head" style="margin-bottom: 20px;">
            <h3 style="margin: 0; color: var(--heading-color, var(--text-color, #fff)); font-size: 1.15rem; font-weight: 600; text-transform: capitalize;">
                        ...
                        <table class="admin-table" style="margin-bottom: 0;">
                            ...
                            <tbody>
                                @foreach($items as $data)
                                    ...
                                    <tr>
                                        <td>
                                            <strong style="color: var(--heading-color, var(--text-color, #fff)); font-size: 0.88rem;">{{ $subLabel }}</strong>
                                        </td>
                                        <td>
                                            <code style="color: var(--brand); font-family: monospace; font-size: 0.8rem;">{{ $data->url_path }}</code>
                                        </td>
                                        <td>
                                            <span style="color: var(--text-color, #cbd5e1); font-size: 0.87rem;">{{ Str::limit($data->meta_title, 45) ?: '-' }}</span>
                                        </td>
                                        <td>
                                            <span style="color: var(--muted, #94a3b8); font-size: 0.82rem;">{{ Str::limit($data->keyword, 35) ?: '-' }}</span>
                                        </td>
                                        <td>
                    ...
                    <tbody>
                        ...
                        @forelse ($metadatas as $data)
                            <tr>
                                ...
                                <td><strong style="color: var(--heading-color, var(--text-color, #fff));">{{ $data->page_name }}</strong></td>
                                <td><code style="color: var(--brand); font-family: monospace; font-size: 0.85rem;">{{ $data->url_path }}</code></td>
                                <td><span style="color: var(--text-color, #cbd5e1); font-size: 0.9rem;">{{ Str::limit($data->meta_title, 45) ?: '-' }}</span></td>
