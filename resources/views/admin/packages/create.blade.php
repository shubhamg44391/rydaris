...
@section('main-content')
    <div class="admin-panel">
        ...
        <div class="panel-body" style="padding: 24px;">
            <form id="packageCreateForm" method="POST" action="{{ route('admin.packages.store') }}" novalidate>
                @csrf
                ...
                <div class="mb-4" style="border: 1px solid var(--line, rgba(255, 255, 255, 0.12)); border-radius: var(--radius); padding: 24px; background: var(--bg-2, #0b1020);">
                    ...
                    <div class="permissions-grid">
                        ...
                        <div class="menu-permission-card">
                            <div style="display: flex; align-items: center; gap: 12px; min-width: 260px; flex-shrink: 0;">
                                <label class="theme-switch">
                                    <input type="checkbox" id="maintenance_schedule_menu" name="maintenance_schedule_menu" value="1" {{ old('maintenance_schedule_menu', '1') ? 'checked' : '' }} />
                                    <span class="slider round"></span>
                                </label>
                            ...
                            <div id="maintenance_schedule_limit_container" class="limit-input-wrapper" style="flex-direction: column; gap: 12px; align-items: flex-end;">
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    ...
                                    <div class="segmented-control">
                                        <input type="radio" name="limit_type_no_of_maintenance_schedules" id="no_of_maintenance_schedules_unlimited" value="unlimited" {{ (old('limit_type_no_of_maintenance_schedules') === 'unlimited') ? 'checked' : '' }} onchange="toggleLimitInputType('no_of_maintenance_schedules', 'unlimited')" />
                                        <label for="no_of_maintenance_schedules_unlimited">Unlimited</label>

                                        <input type="radio" name="limit_type_no_of_maintenance_schedules" id="no_of_maintenance_schedules_limited" value="limited" {{ (old('limit_type_no_of_maintenance_schedules', 'limited') === 'limited') ? 'checked' : '' }} onchange="toggleLimitInputType('no_of_maintenance_schedules', 'limited')" />
                                    ...
                                    <div id="no_of_maintenance_schedules_input_wrapper" style="display: none;">
                                        <input type="number" name="no_of_maintenance_schedules" id="no_of_maintenance_schedules" class="limit-input-field @error('no_of_maintenance_schedules') is-invalid @enderror" value="{{ old('no_of_maintenance_schedules', 0) }}" min="0" placeholder="Limit count" />
                                    </div>
                                </div>
                ...
                <div class="d-flex align-items-center gap-3" style="display: flex; gap: 16px; align-items: center; margin-top: 24px;">
                    <button type="submit" id="submitPkgBtn" class="btn btn-primary rounded-pill px-4" style="min-height: 40px; font-weight: 800; font-size: 0.9rem; background: var(--brand, #52ead2); border: none; color: #061218; cursor: pointer;">Create Package</button>
...
<script>
    $(document).ready(function () {
        ...
        $('#packageCreateForm').on('submit', function (e) {
            e.preventDefault();
            var $form = $(this);
            var $submitBtn = $('#submitPkgBtn');
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    ...
                    if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                        ...
                        var msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'An error occurred while creating the package.';
