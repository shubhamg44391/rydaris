        ...
        <div class="panel-card" style="padding: 24px;">
            <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" class="form-control" placeholder="e.g. Administrator" value="{{ old('name', $role->name) }}" required style="background: #050711; border: 1px solid rgba(255,255,255,0.15); color: #ffffff;">
                    @error('name')
                                        ...
                                        <td style="padding: 12px; text-align: center;">
                                            ...
                                            @else
                                                <input type="checkbox" name="permissions[{{ $key }}][{{ $action }}]" value="1" class="perm-checkbox"
                                                    {{ isset($rolePermissions[$key][$action]) ? 'checked' : '' }}
                ...
                <div style="display: flex; gap: 15px; margin-top: 30px;">
                    ...
                    <button type="submit" class="btn" style="background: linear-gradient(135deg, #52ead2 0%, #80a7ff 100%); color: #051013; padding: 10px 24px; font-weight: 700; border: none; border-radius: 6px; cursor: pointer;">Update Role</button>
                </div>
            </form>
        </div>
    </div>
