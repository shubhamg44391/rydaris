        ...
        <div class="panel-card" style="padding: 24px;">
            <form action="{{ route('admin.admin-users.update', $adminUser->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" class="form-control" placeholder="e.g. John Doe" value="{{ old('name', $adminUser->name) }}" required style="background: #050711; border: 1px solid rgba(255,255,255,0.15); color: #ffffff;">
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" class="form-control" placeholder="e.g. admin@example.com" value="{{ old('email', $adminUser->email) }}" required style="background: #050711; border: 1px solid rgba(255,255,255,0.15); color: #ffffff;">
                    @error('email')
                        <div class="text-danger" style="color: #fb7185; font-size: 0.85rem; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    <label class="form-label" style="color: #f8fafc; font-weight: 600;">PASSWORD</label>
                    <input type="password" name="password" class="form-control" placeholder="Leave blank to keep current password" style="background: #050711; border: 1px solid rgba(255,255,255,0.15); color: #ffffff;">
                    @error('password')
                        <div class="text-danger" style="color: #fb7185; font-size: 0.85rem; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div style="display: flex; gap: 15px; margin-top: 30px;">
                    ...
                    <button type="submit" class="btn" style="background: linear-gradient(135deg, #52ead2 0%, #80a7ff 100%); color: #051013; padding: 10px 24px; font-weight: 700; border: none; border-radius: 6px; cursor: pointer;">Update User</button>
                </div>
            </form>
        </div>
    </div>
